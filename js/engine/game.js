/* ════════════════════════════════════════
   game.js — Versão Unificada e Corrigida
════════════════════════════════════════ */
...
const GameEngine = (() => {
  ...
  function _initialState() {
    return {
      phase: 'idle', themeId: null, levelIndex: null, levelData: null,
      grid: [], words: [], timer: 0, _timerRef: null,
      combo: 1, _comboRef: null, errors: 0, coinsEarned: 0,
    };
  }
  ...
  function _wordMissed() {
    ...
    _cb.onCombo?.(S.combo);
    _cb.onError?.();
    if (S.errors >= S.levelData.maxErrors) _triggerDefeat(`Muitos erros (${S.errors})!`);
  }
  ...
  return { 
    start, on, pause: () => { if(S.phase === 'playing') { S.phase = 'paused'; _clearTimers(); } },
    resume: () => { if(S.phase === 'paused') { S.phase = 'playing'; _startTimer(); } },
    restart: () => start(S.themeId, S.levelIndex) 
  };
})();
